...
const Feedback = ({
  ...
  const dateToString = (date) => {
    ...
    return `${m}월 ${d}일 ${String(h).padStart(2, "0")}:${String(mi).padStart(
      2,
      "0"
    )}`;
  };
  ...
  const tableCell =
    " text-sm text-left border-b-[1px] border-gray-dark text-center";
  ...
  return (
    <div className="flex flex-col w-full p-5 overflow-y-scroll no-scrollbar">
      ...
      <div class="flex flex-row-reverse justify-center">
        <button
          class={`bg-gray-light peer peer-hover:bg-orange hover:bg-orange focus:bg-orange peer-focus:bg-orange rounded-full w-12 h-12 mx-2 ${
            score >= 5 ? "bg-orange" : ""
          }`}
          onClick={() => setscore(5)}
        ></button>
      ...
      <button
        className="mt-4 w-1/5 p-1 bg-orange text-white border border-orange rounded hover:bg-gray-light hover:text-orange"
        onClick={() => {
